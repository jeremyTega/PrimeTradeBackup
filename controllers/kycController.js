...
const approveKyc = async (req, res) => {
    try {
        ...
        const findUser = await kycModel.findOne({ userId: userId }); // Find KYC entry
        const User = await userModel.findOne({ _id: userId }); // Find user entry

        if (!findUser) {
            return res.status(400).json({ message: "User not found" });
        }

        findUser.approved = true; // Update KYC approval status
        await findUser.save(); // Save KYC entry with updated status

        User.kyc.verified = true; // Update user's KYC verification status
        await User.save(); // Save user entry with updated status
...
const rejectKyc = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: `done`, findUser,user });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
